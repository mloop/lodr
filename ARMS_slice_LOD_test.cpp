...
#include "arms_method/arms_test.c"
#include <iostream>
...
vec slice_sample_normal_ex(arma::vec y_data, arma::vec x_data,
  ...
  vec sim_sample = vec(sample_size);
  ...
  while(iteration_index < (max_iterations-1) && iterations_finished == 0){
    ...
    for(int i=0; i <x_data.n_rows ;i=i+1){
      ...
      if( (d.x_preds).has_nan() == FALSE){
        ...
        x_data_return(uvec {row_index_count-1}, find_finite(d.x_preds)) = trans(d.x_preds);
        ...
        for(int s=0; s<no_of_samples; s=s+1){
          ...
          x_data_return(uvec {row_index_count-1},find_finite(d.x_preds))=trans(d.x_preds(find_finite(d.x_preds)));
    ...
    mat x_data_analysis = x_data_return;
    vec weight_vec = x_data_return.col(x_data_return.n_cols-1);
    x_data_analysis.shed_col(x_data_return.n_cols-1);
    vec beta_hat_wls = inv(trans(x_data_analysis)*diagmat(weight_vec)*x_data_analysis)*trans(x_data_analysis)*diagmat(weight_vec)*y_expand;
    beta_estimates.row(iteration_index) = trans(beta_hat_wls);
    
    // Update mean and covariance matrix
    mat weighted_data = x_data_analysis;
    ...
    mat weighted_mean_x_mat = mat(x_data_analysis.n_rows,x_data_analysis.n_cols,fill::zeros);
...
List bootstrap_test(int num_of_boots,
  ...
  for(int j=0; j<num_of_boots; j=j+1){
    ...
    uvec ids=conv_to<uvec>::from(linspace(0, y_data.n_elem-1, 
                                          y_data.n_elem));
    uvec boot_ids = sample(ids, ids.n_elem, true);
    vec y_data_boot = y_data.elem(boot_ids);
    mat x_data_boot = x_data.rows(boot_ids);
    ...
    uvec no_LOD_by_column = {x_data_boot.n_rows+1};
    ...
    mat x_data_boot_complete = x_data_boot;
    vec y_data_boot_complete = y_data_boot;
    ...
    vec beta = inv(trans(x_data_boot_complete)*x_data_boot_complete)*trans(x_data_boot_complete)*y_data_boot_complete;
    ...
    vec mean_x_preds = conv_to<vec>::from(mean(x_data_boot_complete, 0));
    mat sigma_x_preds = cov(x_data_boot_complete, 0);
    boot_results[j] =
...
List LOD_fit_multiple(arma::vec y_data, arma::mat x_data, 
  ...
  vec ids_LOD=zeros(x_data.n_rows);
   for(int i=0; i<x_data.n_rows; i=i+1){
  ...
  vec y_expand=zeros(no_of_samples*accu(ids_LOD)+x_data.n_rows-accu(ids_LOD));
  vec diff = zeros(beta.n_elem)+threshold+1.0;
  mat x_data_return=mat(no_of_samples*accu(ids_LOD)+x_data.n_rows-accu(ids_LOD),x_data.n_cols+1,fill::zeros);
  ...
  while(iteration_index < (max_iterations-1) && iterations_finished == 0){
     ...
     for(int i=0; i <x_data.n_rows ;i=i+1){
       ...
       if( (d.x_preds).has_nan() == FALSE){
         ...
         uvec missing_indicies = find_nonfinite(d.x_preds);
            ...
            /* Sample using slice sampler */ 
              ...
              if(sampler==0){
                  if(s==0){
                    for(int l=0; l<missing_indicies.n_elem; l=l+1){
                      ...
                      double burn_in_inital = slice_fcs(norm_multi, d, xprev(d.missing_pred), 
                                                 as_scalar(LOD_u_l(uvec {d.missing_pred}, uvec {0})), as_scalar(LOD_u_l(uvec {d.missing_pred}, uvec {1})));
                    ...
                    for(int l=0; l<missing_indicies.n_elem; l=l+1){
                      ...
                      xsamp=slice_fcs(norm_multi, d, xprev(d.missing_pred), 
                                      as_scalar(LOD_u_l(uvec {d.missing_pred}, uvec {0})), as_scalar(LOD_u_l(uvec {d.missing_pred}, uvec {1})), 1);
                ...
                for(int l=0; l<missing_indicies.n_elem; l=l+1){
                  ...
                  d.missing_pred = missing_indicies(l);  
                  int ninit = 4;
                  double xl = as_scalar(LOD_u_l(uvec {d.missing_pred}, uvec {0})), xr=as_scalar(LOD_u_l(uvec {d.missing_pred}, uvec {1}));
                  int dometrop = 1;
                  double xprev;
                  if(xr<0){ 
                    xprev=xr*sqrt(2);
...
List bootstrap_multi_test(int num_of_boots,
  ...
  for(int j=0; j<num_of_boots; j=j+1){
    ...
    no_LOD_by_column.shed_row(0);
    uvec no_LOD_by_column_unique = unique(no_LOD_by_column);
